...
public class MoveRoomB : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        battleSystem.moveB = false; // Initialize BattleSystem flag for this room
    ...
    private void Update()
    {
        ...
        if (battleSystem.state == BattleState.WON && !isMoving)
        {
            ...
            // Calculate target position in front of current position
            targetMovePosition = transform.position + transform.forward * moveDistance;

            isMoving = true;       // Start moving
            battleSystem.moveB = false; // Reset BattleSystem flag
